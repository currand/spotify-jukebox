 ...
 *   - Server running at https://jukebox.currannet.net
...
async function api(
  ...
  const res = await fetch(`https://jukebox.currannet.net${path}`, {
    method,
    headers,
...
async function guestLoop(
  ...
  if (savedGuest && savedGuest.cookie) {
    ...
    log({ time: new Date().toISOString(), guest: guest.name, action: "reused", detail: ("id=" + (guest.guestId ?? "").slice(0, 8)) });
  } else {
  // ── Step 1: Join ──────────────────────────────────────────────────────
    ...
    try {
      switch (action) {
        ...
        case "add": {
          ...
          if (searchRes.status !== 200 || !searchRes.json.tracks?.length) break;
          const track = pickRandom(searchRes.json.tracks);
...
async function main() {
  ...
  const guestFilePath = "./data/guests-" + config.partySlug + ".json";
  try {
    const { readFileSync } = await import("fs");
    savedGuests = JSON.parse(readFileSync(guestFilePath, "utf8"));
    console.log("Loaded " + savedGuests.length + " pre-joined guests from " + guestFilePath);
  } catch {
    console.log("No saved guests found — will join fresh");
  }
  ...
  // Phase definitions: realistic party arc
  const PHASES = [
    { name: "Arrival", durationMs: 60 * 60 * 1000, activityRate: 0.35, searchBias: 1.5 },   // 60 min: guests arrive, search & add
    { name: "Peak Party", durationMs: 90 * 60 * 1000, activityRate: 0.15, searchBias: 0.5 }, // 90 min: upvoting, vetoing
    { name: "Winding Down", durationMs: 30 * 60 * 1000, activityRate: 0.08, searchBias: 0.3 }, // 30 min: mostly idle
  ];
  ...
  // Stagger guest arrivals: 1 every ~2min over the first hour
  const arrivalIntervalMs = (60 * 60 * 1000) / guests.length;
  const guestPromises: Promise<void>[] = [];
  for (let i = 0; i < guests.length; i++) {
    if (i > 0) {
      const delay = randomDelay(arrivalIntervalMs * 0.5, arrivalIntervalMs * 1.5);
  ...
  console.log(("\n🚀 " + guests.length + " guests launched with staggered arrivals.").trim());
  console.log(("  Phases: " + PHASES.map((p) => p.name + " (" + (p.durationMs / 60000) + "min)").join(" → ")).trim() + "\n");
  ...
  await Bun.sleep(totalDurationMs);
  shutdown = true;
