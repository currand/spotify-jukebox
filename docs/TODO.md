...
- [ ] Optional artist field on guest search to disambiguate common track titles (e.g. `track:Black` + `artist:Pearl Jam`). Keep a single primary box for party UX; use Spotify field syntax server-side when artist is provided.
- [ ] Post-search "filter by artist" chip if ambiguous results are common in production.
